...
public class ParticleSwarmOptimization {
	...
	public ParticleSwarmOptimization(int n) {
		MAX_LENGTH = n;
		PARTICLE_COUNT = 40;	//taille de population
		V_MAX = 12; /* 4 8 12 16 20*/
		MAX_EPOCHS = 10000; /* 1000 5000 10000 50000 100000*/
		TARGET = 0;
		SHUFFLE_RANGE_MIN = 8;
	...
	public boolean algorithm() {
			...
			if(epoch < MAX_EPOCHS) {
	            for(int i = 0; i < PARTICLE_COUNT; i++)  {
	                ...
	                if(aParticle.getConflicts() == TARGET){
	                	...
	                	if(aParticle.getConflicts() < max) {
	                		max = aParticle.getConflicts();
	                		sol = aParticle;
	                	}
	                }
	            } // i
	            
	            Collections.sort(particles); 	//TRIE				// sort particles by their conflicts scores, best to worst.
		...
		if(epoch == MAX_EPOCHS) {
			System.out.println("No solution found"+sol);
			done = false;
		}
	...
	public void updateParticles() {
	    ...
	    for(int i = 1; i < PARTICLE_COUNT; i++) {
        	...
        	for(int j = 0; j < changes; j++) {
        		if(new Random().nextBoolean()) { //exploitation
        			randomlyArrange(i); //crossover
        		}
        		// Push it closer to it's best neighbor.
        		copyFromParticle(source, destination); //exploitation..mutation
        	} // j
