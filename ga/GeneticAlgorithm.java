...
public class GeneticAlgorithm {
	...
	private int MIN_SELECT;                 // Minimum nbr parents allowed for selection.
	private int MAX_SELECT;                 // Maximum nbr parents allowed for selection. Range: MIN_SELECT < MAX_SELECT < START_SIZE
	...
	public GeneticAlgorithm(int n) {
		MAX_LENGTH = n;
		START_SIZE = 40; //c'est bon
		MAX_EPOCHS = 10000;
		MATING_PROBABILITY = 0.70;  //c'est bon
	...
	public boolean algorithm() {
		...
		while(!done) {
			...
			for(int i = 0; i < populationSize; i++) {
				...
				if((thisChromo.getConflicts() == 0)) {			//si solution found
					done = true;
				}else {
					if(thisChromo.getConflicts() < max) {
						max = thisChromo.getConflicts();
						sol = thisChromo;
					}
				}
			}

			if(epoch == MAX_EPOCHS) {							//si Max Number of Cycles 
				done = true;
			}
			...
			//methode de selection par roulette: avec setSelectionProbability associe une proba de selection a chaque chromosome
			rouletteSelection(); // 

			//le croisement (crossover) et la mutation (mutation) des individus.
			mating();
		...
		if(epoch >= MAX_EPOCHS) {
			...
			for(int i=0;i<sol.getMaxLength();i++) {
				System.out.println("solution found"+sol.getGene(i));
			}
			done = false;
		} else {
	...
	public void mating() {
        ...
        //OFFSPRING_PER_GENERATION = nbr chromosome a muter
        for(int i = 0; i < OFFSPRING_PER_GENERATION; i++) {
            ...
            if(getRand <= MATING_PROBABILITY * 100) {
                ...
                if(childCount - 1 == nextMutation) {
                    exchangeMutation(newIndex1, 1);  //1 nbr de mutation
	...
	public void initialize() {
		...
		for(int i = 0; i < START_SIZE; i++)  {
			...
			shuffles = getRandomNumber(MINIMUM_SHUFFLES, MAXIMUM_SHUFFLES);//nbr de mutation
	...
	public void exchangeMutation(int index, int exchanges) {
		...
		//boucle pour le nbr de mutation
   ...
   /* gets the solutions
	 ...
	 * @return: solutions
	 */  
	public static ArrayList<Chromosome> getSolutions() {
		return solutions;
	}
